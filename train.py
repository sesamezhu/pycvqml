...
class ShapesConfig(Config):
    ...
    重定义自己的数据集类
    '''
    NAME = "hook"    # 改为自己的数据集名称
    GPU_COUNT = 1    # 使用1个GPU
    IMAGES_PER_GPU = 1  # 每个GPU处理1张图片
    #BATCH_SIZE = GPU_COUNT*IMAGES_PER_GPU  
    NUM_CLASSES = 1 + 2   # 1+训练的目标类别数
    
    # anchor面积以及图片resize的尺寸
    # 使用该参数训练处的模型，只能在该参数下进行predict
...
if __name__ == "__main__":
    ...
    init_epoch = 0        # 初始epoch，如果第一次训练则是0；如果是“继续训练”则根据已有的checkPoint进行修改
    epoch = 1     # 共训练多少epoch
    
    # 训练数据文件夹
    dataset_root_path="E:/MaskR-CNN_TensorFlow/train_dataset/train/"
    img_floder = dataset_root_path + "imgs/"
    mask_floder = dataset_root_path + "mask/"
    yaml_floder = dataset_root_path + "yaml/"
    imglist = os.listdir(img_floder)  # 获取imgs内图片名list
    ...
    val_imglist = imglist[int(count*0.1):]
...
#################################################################################################################
    ...
    #print(dataset_train.image_ids)
    #print(train_generator.image_ids)
